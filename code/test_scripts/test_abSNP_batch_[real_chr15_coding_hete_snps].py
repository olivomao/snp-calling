...
def main():
    ...
    dstDir = '/data1/shunfu1/SNPCalling/data_real_HeteSNPsCodingRegion/'
    ...
    trBedFile = '%s/gencode.v19.annotation.target.chr15.sorted.bed'%srcDir
    gtfFile = '%s/gencode.v19.annotation.gtf'%srcDir     

    exp_path = ['%s/exp_m_Reassigned.txt'%srcDir, 
                '%s/exp_p_Reassigned.txt'%srcDir]

    true_SNP = ['%s/chr15_snp_m_codingRegion_noHomo.txt'%srcDir, 
                '%s/chr15_snp_p_codingRegion_noHomo.txt'%srcDir]

    genomeFile = ['%s/chr15_m_HeteSNPs.fa'%srcDir,
                  '%s/chr15_p_HeteSNPs.fa'%srcDir]
