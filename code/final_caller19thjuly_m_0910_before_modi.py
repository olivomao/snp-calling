...
def JP(Qi,Na,Nc,Ng,Nt,R,Lamda_Knot,Lamda,X,Y):
    ...
    for q in range(0,N):
        ...
        Prob_logq =Prob_funct(Qi[q],R,Lamda_Knot,Lamda[q],X,Y[q])
...
def Prob_funct(Qi,R,Lamda_Knot,Lamda,X,Y):
...
#        j = Lamda_T
    ...
    if en_debug==1: #handel exception condition #en_debug_0811
        ...
        if (X==Y)and(Y==R):
            ...
            Q = ((Qi*Lamda_Knot) + j)/tot_Lamda
            
        elif (Y!=X)and(Y!=R):
            
            Q = (((1-Qi)*Lamda_Knot/3) + j)/tot_Lamda   
        
        else:
            
            Q = ((Qi*Lamda_Knot/2) + j)/tot_Lamda
...
def process_count_line(row):
    ...
    prob_A = (JP(Qi,Na,Nc,Ng,Nt,Ru,Lamda_Knot,Lamda,'A',Y))

    prob_C = (JP(Qi,Na,Nc,Ng,Nt,Ru,Lamda_Knot,Lamda,'C',Y))

    prob_G = (JP(Qi,Na,Nc,Ng,Nt,Ru,Lamda_Knot,Lamda,'G',Y))

    prob_T = (JP(Qi,Na,Nc,Ng,Nt,Ru,Lamda_Knot,Lamda,'T',Y))
...
if __name__ == "__main__":
    ...
    final_caller(tophat_dir + count_fn,
                 '/caller_output_alt_mapping_debug_90346907.txt',
                 '/caller_output_exception_alt_mapping_debug_90346907.txt',
                 '/caller_output_snp_found_alt_mapping_debug_90346907.txt')
