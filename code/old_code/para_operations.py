...
'''
#it seems count_alt format is changed, so we modify this func into merge_line_list_altInfo2
...
def merge_line_list_altInfo2(line_list, curr_count_y_file):
...
def merge_count_x_altInfo(count_x_dir, #split_sam_dir + count_split_dir0,
    ...
    for i in range(num_tot_pos):
        ...
        for j in range(num_f):
            line_list.append(count_x_file_list[j].readline())
        merge_line_list_altInfo2(line_list, curr_count_y_file)
    #pdb.set_trace()###
...
if __name__ == "__main__":
    ...
    if len(sys.argv)<2: #simple debug and test
        ...
        'python para_operations.py para_count_merge '+\
                  [2]split_sam_dir+count_split_dir0+' '\
                  [3]'count_x'+' '+\
                  [4]'region_00.txt'+' '+\
                  [5]repr(num_p)+' '+\
                  [6]Default_Ref_Path + tophat_dir + count_split_dir1 + ' '+\
                  [7]'count_y00.txt'
        """
        mode = sys.argv[1]
