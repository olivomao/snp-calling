...
#try to
#filter away alt mappings w/ same abundance
#add a tuning parameter at filtering decision in order to get roc perf
def filter_snp_lam_half_filt3(snp_res_address,
                              #filt_snp_res_address,
                              count_altInfo_address,
                              count_abs_address,
                              filtSameAb=True,
                              rocThre=[1.0]): # check shadow snp from counts_altInfo.txt
    
    bases = {"A": 0, "C": 1, "G": 2, "T": 3}
    
    print('lambda half filtering 3 (alt count modified; add filtSameAb and rocThre)')
    #pdb.set_trace()
    ...
    #pdb.set_trace()
    possible_snps_to_remove = {} #key: gp val: [[gp0, l0, pmf0], [gp_alt1, l_alt1, pmf_alt1], ...]
    nLines=sum([1 for l in open(count_abs_address,'r')]); ProgT=nLines/100; ProgP=0; ProgQ=0;
    with open(count_altInfo_address) as count_altInfo_file, open(count_abs_address) as count_file:
        ...
        for xline, yline in izip(count_altInfo_file, count_file):
            ...
            ProgP += 1
            if ProgP>=ProgT: ProgP=0; ProgQ+=1; sys.stdout.write('\r'); sys.stdout.write('%d %% processed (filter snp 3 countFile)'%ProgQ); sys.stdout.flush()
            ...
            if len(x)>=3:
                #pdb.set_trace()
                #e.g. x=['915755', '43893171,4.346', '[-G:43992989,0.0,][+G:43992989,0.0,]']
                #e.g. y=['915755', '43893171', 'C', '4.346', '0', '0', '5', '0', 'G,I,0.00,4.35', 'G,I,0.00,4.35', 'G,I,0.00,4.35', 'G,I,0.00,4.35', 'G,I,0,4.35']
                # found_snps[gp]=[['C', 'G', '']], N_snp_base=5
                ...
                gp = q
                #if gp==28634473: pdb.set_trace()
                possible_snps_to_remove[gp]=[]
                possible_snps_to_remove[gp].append([q, q_e, pmf])
                
                #add alt mappings - find (pos,lamda) pairs
                pos_el_dic = {}
                itms = x[2].split('[')
                for itm in itms:
                ...
                for q, q_e in pos_el_dic.iteritems():
                    pmf = poisson(q_e/2.0).pmf(N_snp_base)
                    #if q==28634473: pdb.set_trace()
                    possible_snps_to_remove[gp].append([q, q_e, pmf])
                
    #del snp which has same lambda at alt locations
    print('')

    for rocT in rocThre:
        
        filt_snp_res_addr = snp_res_address[:-4]+'_rocT_%.2f.txt'%rocT

        found_snps2 = copy.deepcopy(found_snps)
        possible_snps_to_remove2 = copy.deepcopy(possible_snps_to_remove)

        for gp, q_qe_pmf_list in possible_snps_to_remove2.items():
            ...
            if filtSameAb == True:
                ...
                sameAb = True
                for i in xrange(1, len(q_qe_pmf_list)):
                    qe_prev = q_qe_pmf_list[i-1][1]
                    qe_curr = q_qe_pmf_list[i][1]
                    sameAb = sameAb and (float(abs(qe_curr-qe_prev))/qe_prev<0.005)
                if len(q_qe_pmf_list)>1 and sameAb==True: #snp at gp has alt loc and not distinguishable from its alt loc
                    #pdb.set_trace()
                    to_remove = True

            #rule 2
            pmf0 = q_qe_pmf_list[0][2]
            pmf_alt = [q_qe_pmf_list[i][2] for i in xrange(1, len(q_qe_pmf_list))] 
            if to_remove == False and pmf_alt != [] and max(pmf_alt)>=rocT*pmf0: #rocT=1: as filt2; rocT=0: min fp; rocT=+inf: min md; '>=' is necessary here 
                #pdb.set_trace()
                to_remove = True
        ...
        #output
        print('write to %s...'%filt_snp_res_addr)
        with open(filt_snp_res_addr, 'w') as fo:

            for itm in found_snps2.items():
                #pdb.set_trace()
                gp = itm[0]
                rB = itm[1][0][0]
                tB = itm[1][0][1]
