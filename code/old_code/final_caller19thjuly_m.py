...
'''
def merge_caller_res(caller_op_x,  caller_op_exception_x,  caller_op_snp_x, num_p, #inputs
                     caller_op_fn, caller_op_exception_fn, caller_op_snp_fn,
                     outDir=Default_Ref_Path,
                     clear=0): #outputs
    
    if outDir in caller_op_fn: #abs address
        caller_op_file = open(caller_op_fn, 'w')
    else:
        caller_op_file = open(outDir+caller_op_fn, 'w')
    for i in range(num_p):
        caller_op_x_fn = '%s%02d.txt'%(caller_op_x, i)
        #if outDir not in caller_op_x_fn:
        #caller_op_x_fn = outDir + caller_op_x_fn
        ...
        if clear==1:
            #pdb.set_trace()
            cmd = 'rm %s'%caller_op_x_fn            
            run_cmd(cmd)
    caller_op_file.close()
    
    if outDir in caller_op_exception_fn:
        caller_op_exception_file = open(caller_op_exception_fn, 'w')
    else:
        caller_op_exception_file = open(outDir+caller_op_exception_fn, 'w')
    for i in range(num_p):
        caller_op_exception_x_fn = '%s%02d.txt'%(caller_op_exception_x, i)
        #if outDir not in caller_op_exception_x_fn:
        #caller_op_exception_x_fn = outDir + caller_op_exception_x_fn    
        ...
        if clear==1:
            #pdb.set_trace()
            cmd = 'rm %s'%caller_op_exception_x_fn
            run_cmd(cmd)

    caller_op_exception_file.close()
    
    if outDir in caller_op_snp_fn:
        caller_op_snp_file = open(caller_op_snp_fn, 'w')
    else:
        caller_op_snp_file = open(outDir+caller_op_snp_fn, 'w')
    for i in range(num_p):
        caller_op_snp_x_fn = '%s%02d.txt'%(caller_op_snp_x, i)
        #if outDir not in caller_op_snp_x_fn:
        #caller_op_snp_x_fn = outDir + caller_op_snp_x_fn
...
'''
#use external file
def merge_caller_res1(caller_op_x,  caller_op_exception_x,  caller_op_snp_x, num_p, #inputs
                      caller_op_fn, caller_op_exception_fn, caller_op_snp_fn): #outputs
